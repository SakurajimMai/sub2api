...
// batchRows 是 BatchSnapshotUsage 每批最大行数（9 参/行 × 6000 + 1 ≈ 54001 参,低于 Postgres 65535 上限）。
const batchRows = 6000

// BatchSnapshotUsage 用一条多行 UPDATE ... FROM (VALUES ...) 把整批 usage 以绝对值覆盖写入既有活跃行（非累加）。
// 每批最多 batchRows 行；$1=now 共用；每行 9 个 per-row 参（user_id, platform, 3×usage, 3×window_start, weekly_generation）。
// 只命中 deleted_at IS NULL 的既有行：行不存在或已软删的 (user, platform) 直接跳过，不建行。
// 周窗口按 weekly_quota_generation 单调前进，旧代次快照不能覆盖新代次用量。
//
...
func (r *userPlatformQuotaRepository) BatchSnapshotUsage(ctx context.Context, snapshots []UserPlatformQuotaSnapshot, now time.Time) error {
	...
	for start := 0; start < len(snapshots); start += batchRows {
		...
		query, args := buildUserPlatformQuotaSnapshotUpdate(batch, now)
...
func buildUserPlatformQuotaSnapshotUpdate(batch []UserPlatformQuotaSnapshot, now time.Time) (string, []any) {
	var sb strings.Builder
	_, _ = sb.WriteString(
		"UPDATE user_platform_quotas AS q SET" +
			"  daily_usage_usd      = v.daily_usage_usd," +
			"  weekly_usage_usd     = CASE WHEN v.weekly_quota_generation > q.weekly_quota_generation" +
			" OR (v.weekly_quota_generation = q.weekly_quota_generation" +
			" AND (q.weekly_window_start IS NULL" +
			" OR v.weekly_window_start > q.weekly_window_start))" +
			" THEN v.weekly_usage_usd" +
			" WHEN v.weekly_quota_generation = q.weekly_quota_generation" +
			" AND v.weekly_window_start = q.weekly_window_start" +
			" THEN GREATEST(q.weekly_usage_usd, v.weekly_usage_usd)" +
			" ELSE q.weekly_usage_usd END," +
			"  monthly_usage_usd    = v.monthly_usage_usd," +
			"  daily_window_start   = v.daily_window_start," +
			"  weekly_window_start  = CASE WHEN v.weekly_quota_generation > q.weekly_quota_generation" +
			" OR (v.weekly_quota_generation = q.weekly_quota_generation" +
			" AND (q.weekly_window_start IS NULL" +
			" OR v.weekly_window_start > q.weekly_window_start))" +
			" THEN v.weekly_window_start ELSE q.weekly_window_start END," +
			"  weekly_quota_generation = GREATEST(q.weekly_quota_generation, v.weekly_quota_generation)," +
			"  weekly_reserved_generation = GREATEST(q.weekly_reserved_generation, v.weekly_quota_generation)," +
	...
	for i, s := range batch {
		...
		fmt.Fprintf(&sb, "($%d::bigint,$%d::varchar,$%d::numeric,$%d::numeric,$%d::numeric,$%d::timestamptz,$%d::timestamptz,$%d::timestamptz,$%d::bigint)",
	...
	_, _ = sb.WriteString(
		") AS v(user_id, platform, daily_usage_usd, weekly_usage_usd, monthly_usage_usd," +
			" daily_window_start, weekly_window_start, monthly_window_start, weekly_quota_generation)" +
